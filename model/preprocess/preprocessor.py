...
class Preprocessor:
    ...
    def subzero_temperature_mean(self, temperatures: np.ndarray) -> float:
        return self.array_mean(temperatures[temperatures < 0])
    ...
    def transit_during_rainfall(self, df: pd.DataFrame, is_raining: str = FeatureName.IS_RAINING,
    ...
    def transit_heavy_during_rainfall(self, df: pd.DataFrame, is_raining: str = FeatureName.IS_RAINING,
                                      length_name: str = FeatureName.HEAVY_VEHICLES_TRANSIT_TOTAL) -> int:
    ...
    def is_raining_at_time(self, data: np.ndarray, timestamp: datetime, delta: timedelta = timedelta(minutes=5)):
    ...
    def process(self, raw_dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        dataset[FeatureName.IS_RAINING] = np.array(
            [1 if self.is_raining_at_time(raw_dataset[RawFeatureName.RAINFALL.value], pd.to_datetime(timestamp)) else 0 for timestamp in dataset.index]
        )

        dataset[FeatureName.HEAVY_VEHICLES_TRANSIT_TOTAL] = self.array_count(heavy_transit)
        dataset[FeatureName.TRANSIT_DURING_RAINFALL] = self.transit_during_rainfall(dataset)
        dataset[FeatureName.HEAVY_VEHICLES_TRANSIT_DURING_RAINFALL] = self.transit_heavy_during_rainfall(dataset)
